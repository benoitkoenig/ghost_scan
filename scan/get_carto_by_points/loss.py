...
def loss(gt, pr):
  ...
  maskedPr = mask * (pr + 1e-5) # 1e-5 is here to prevent the loweest pr to converge to 0, forcing the highest pr to converge to 1 instead. 1e-5 times all the mask's pixels is close to 1
