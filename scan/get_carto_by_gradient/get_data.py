...
def getXY(filenames):
  X = loadPngTensors(['%s/data/training/printed_document_without_background/%s' % (dirpath, f) for f in filenames], h, w)
  Y = loadPngTensors(['%s/data/training/printed_gradient_map/%s' % (dirpath, f) for f in filenames], h, w)[:, :, :, 0:3]
  return X, Y
