...
def add_force_fields():
  ...
  bpy.data.objects['Field.002'].field.strength = random.uniform(0, 3)
  if (random.random() < 0.5): # Then the force field goes in a corner
    ...
    bpy.data.objects['Field.002'].location = [random.uniform(cornerSign1 * 2.5, cornerSign1 * 2), random.uniform(cornerSign2 * 2, cornerSign2 * 1.5), random.uniform(-1, -0.2)]
  else: # Then the force field goes near the center
    bpy.data.objects['Field.002'].location = [random.uniform(-2, 2), random.uniform(-1.5, 1.5), random.uniform(-1, -0.2)]
